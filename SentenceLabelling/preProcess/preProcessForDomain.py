...
def extract_conversation(train_path, data_path, isTest):
    global dic
    ...
    for index, row in df.iterrows():
        if pd.isnull(row['utterance']) and len(temp_df) > 1:
            ...
            # Store test words in the dictionary
            if isTest:
                words = sentence.split()
                for word in words:
                    if word not in dic:
...
def run_pre_process(train_path, data_path, isTest):

    extract_conversation(train_path, data_path, isTest)
